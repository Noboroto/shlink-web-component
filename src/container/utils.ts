...
export function useDependencies<Deps>(
  obj: Deps
): Omit<Required<Deps>, keyof FC> {
...
export function componentFactory<
  Deps,
  CompType = Omit<Partial<Deps>, keyof FC>,
>(Component: CompType, deps: ReadonlyArray<keyof CompType>) {
  return (container: IContainer, console = globalThis.console) => {
    deps.forEach((dep) => {
      ...
      if (!resolvedDependency && process.env.NODE_ENV !== 'production') {
        console.error(
          `[Debug] Could not find "${dep as string}" dependency in container`
        );
      }
