...
export const useShortUrlsQuery = (): [ShortUrlsQuery, ToFirstPage] => {
  ...
  const filtering = useMemo((): ShortUrlsQuery => {
    ...
    const parsedOrderBy = orderBy
      ? stringToOrder<ShortUrlsOrderableFields>(orderBy)
      : undefined;
  ...
  const toFirstPageWithExtra = useCallback(
    (extra: Partial<ShortUrlsQuery>) => {
      ...
      const newQuery: ShortUrlsRawQuery = {
        ...
        excludeMaxVisitsReached: parseOptionalBooleanToString(
          excludeMaxVisitsReached
        ),
        excludePastValidUntil: parseOptionalBooleanToString(
          excludePastValidUntil
        ),
      };
...
export const useShortUrlIdentifier = (): ShortUrlIdentifier => {
  ...
  return useMemo(
    () => ({ shortCode: urlDecodeShortCode(shortCode), domain }),
    [domain, shortCode]
  );
};
