 ...
 */
export const urlEncodeShortCode = (shortCode: string): string =>
  shortCode.replaceAll('/', '__');
 ...
 */
export const urlDecodeShortCode = (shortCode: string): string =>
  shortCode.replaceAll('__', '/');

/**
 * String representation of a short URL, so that it can be used as query param
 */
export const shortUrlToQuery = ({
  domain,
  shortCode,
...
export const queryToShortUrl = (shortUrlQuery: string): ShortUrlIdentifier => {
  ...
  if (!shortCode) {
    throw new Error(
      `It was not possible to parse domain and short code from "${shortUrlQuery}"`
    );
  }
