...
export const deleteShortUrl = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    `${REDUCER_PREFIX}/deleteShortUrl`,
    async ({
      shortCode,
      domain,
    }: ShortUrlIdentifier): Promise<ShortUrlIdentifier> => {
      await apiClientFactory().deleteShortUrl(shortCode, domain);
      return { shortCode, domain };
    }
  );

export const shortUrlDeleted = createAction<ShlinkShortUrl>(
  `${REDUCER_PREFIX}/shortUrlDeleted`
);

export const shortUrlDeletionReducerCreator = (
  ...
  const { actions, reducer } = createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(deleteShortUrlThunk.pending, (state) => ({
        ...state,
        loading: true,
        error: false,
        deleted: false,
