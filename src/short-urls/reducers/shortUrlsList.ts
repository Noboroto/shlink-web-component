...
export const listShortUrls = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    `${REDUCER_PREFIX}/listShortUrls`,
    (params: ShlinkShortUrlsListParams | void): Promise<ShlinkShortUrlsList> =>
...
export const shortUrlsListReducerCreator = (
  ...
  createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(listShortUrlsThunk.pending, (state) => ({
        ...state,
        loading: true,
        error: false,
      }));
      builder.addCase(listShortUrlsThunk.rejected, () => ({
        loading: false,
        error: true,
      }));
      builder.addCase(
      ...
      builder.addCase(createShortUrlThunk.fulfilled, (state, { payload }) => {
        ...
        state.shortUrls.data = [
          payload,
          ...state.shortUrls.data.slice(0, ITEMS_IN_OVERVIEW_PAGE - 1),
        ];
      ...
      builder.addCase(
        ...
        (state, { payload: editedShortUrl }) => {
          ...
          state.shortUrls.data = state.shortUrls.data.map((shortUrl) => {
            ...
            return shortUrlMatches(shortUrl, shortCode, domain)
              ? editedShortUrl
              : shortUrl;
          });
        }
      ...
      builder.addCase(shortUrlDeleted, (state, { payload }) => {
        ...
        state.shortUrls.data = state.shortUrls.data.filter(
          (shortUrl) =>
            !shortUrlMatches(shortUrl, payload.shortCode, payload.domain)
        );
        state.shortUrls.pagination.totalItems -= 1;
      });
