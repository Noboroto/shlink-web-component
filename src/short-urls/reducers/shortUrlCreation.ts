...
export const createShortUrl = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    `${REDUCER_PREFIX}/createShortUrl`,
    (data: ShlinkCreateShortUrlData): Promise<ShlinkShortUrl> =>
      apiClientFactory().createShortUrl(data)
  );

export const shortUrlCreationReducerCreator = (
  ...
  const { reducer, actions } = createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(createShortUrlThunk.pending, () => ({
        saving: true,
        saved: false,
        error: false,
      }));
      builder.addCase(createShortUrlThunk.rejected, (_, { error }) => ({
        saving: false,
        saved: false,
        error: true,
