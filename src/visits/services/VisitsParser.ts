...
const visitHasProperty = (
  ...
  propertyName: keyof NormalizedVisit
) => visit[propertyName] !== undefined;

const optionalNumericToNumber = (
...
const updateBrowsersStatsForVisit = (
  browsersStats: Stats,
  { browser }: NormalizedVisit
) => {
  browsersStats[browser] = (browsersStats[browser] || 0) + 1;
};

const updateReferrersStatsForVisit = (
  referrersStats: Stats,
  { referer: domain }: NormalizedVisit
) => {
  referrersStats[domain] = (referrersStats[domain] || 0) + 1;
};

const updateLocationsStatsForVisit =
  (propertyName: 'country' | 'city') =>
  (stats: Stats, visit: NormalizedVisit) => {
...
const updateCitiesForMapForVisit = (
  citiesForMapStats: Record<string, CityStats>,
  visit: NormalizedVisit
) => {
  ...
  const currentCity = citiesForMapStats[city] || {
    ...
    count: 0,
    latLong: [
      optionalNumericToNumber(latitude),
      optionalNumericToNumber(longitude),
    ],
  };
...
const updateVisitedUrlsForVisit = (
  visitedUrlsStats: Stats,
  visit: NormalizedVisit
) => {
