...
export const deleteShortUrlVisits = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    `${REDUCER_PREFIX}/deleteShortUrlVisits`,
    async ({
      shortCode,
      domain,
    }: ShortUrlIdentifier): Promise<DeleteVisitsResult> => {
      const result = await apiClientFactory().deleteShortUrlVisits(
        shortCode,
        domain
      );
...
export const shortUrlVisitsDeletionReducerCreator = (
  ...
  createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(deleteShortUrlVisitsThunk.pending, (state) => ({
        ...state,
        deleting: true,
        error: false,
      }));
      builder.addCase(
        deleteShortUrlVisitsThunk.rejected,
        (state, { error }) => ({
          ...state,
          deleting: false,
          error: true,
