...
export const getNonOrphanVisits = (apiClientFactory: () => ShlinkApiClient) =>
  createVisitsAsyncThunk({
    ...
    createLoaders: ({ options }) => {
      ...
      const visitsLoader = async (query: ShlinkVisitsParams) =>
        apiClient.getNonOrphanVisits(query);
      const lastVisitLoader = lastVisitLoaderForLoader(
        doIntervalFallback,
        (q) => apiClient.getNonOrphanVisits(q)
      );
...
export const nonOrphanVisitsReducerCreator = (
  ...
  createVisitsReducer({
    ...
    filterCreatedVisits: ({ params }, createdVisits) => {
      ...
      return createdVisits.filter(({ visit }) =>
