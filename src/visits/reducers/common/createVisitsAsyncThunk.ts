...
import {
  isMandatoryStartDateRangeParams,
  paramsForPrevDateRange,
  toApiParams,
} from '../../helpers';
...
export const createVisitsAsyncThunk = <T extends LoadVisits = LoadVisits>({
  ...
  const fallbackToInterval = createAction<DateInterval>(
    `${typePrefix}/fallbackToInterval`
  );

  const asyncThunk = createAsyncThunk(
    typePrefix,
    async (param: T, { getState, dispatch }): Promise<VisitsLoaded> => {
      ...
      const progresses = queryForPrevVisits
        ? { main: 0, prev: 0 }
        : { main: 0 };
      const computeProgress = (
        key: keyof typeof progresses,
        progress: number
      ) => {
