...
const matchesType = (
  visit: ShlinkOrphanVisit,
  orphanVisitsType?: ShlinkOrphanVisitType
) => !orphanVisitsType || orphanVisitsType === visit.type;

const filterOrphanVisitsByType = (
  { data, ...rest }: ShlinkVisitsList,
  type?: ShlinkOrphanVisitType
) => {
  const visits = data.filter(
    (visit) => isOrphanVisit(visit) && matchesType(visit, type)
  );
  return { ...rest, data: visits };
};

export const getOrphanVisits = (apiClientFactory: () => ShlinkApiClient) =>
  createVisitsAsyncThunk({
...
export const orphanVisitsReducerCreator = (
  ...
  createVisitsReducer({
    ...
    extraReducers: (builder) => {
      builder.addCase(deleteOrphanVisitsThunk.fulfilled, (state) => ({
        ...state,
        visits: [],
      }));
    },
    filterCreatedVisits: ({ params }, createdVisits) => {
      ...
      return createdVisits.filter(
        ({ visit, shortUrl }) =>
          !shortUrl && isBetween(visit.date, startDate, endDate)
      );
    },
  });
