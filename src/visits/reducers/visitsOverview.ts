...
const countBots = (visits: CreateVisit[]) =>
  visits.filter(({ visit }) => visit.potentialBot).length;

export const loadVisitsOverview = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    `${REDUCER_PREFIX}/loadVisitsOverview`,
    (): Promise<ParsedVisitsOverview> =>
      apiClientFactory()
        .getVisitsOverview()
        .then(
          ({
...
export const visitsOverviewReducerCreator = (
  ...
  createSlice({
    ...
    extraReducers: (builder) => {
      builder.addCase(loadVisitsOverviewThunk.pending, () => ({
        ...initialState,
        loading: true,
      }));
      builder.addCase(loadVisitsOverviewThunk.rejected, () => ({
        ...initialState,
        error: true,
      }));
      builder.addCase(loadVisitsOverviewThunk.fulfilled, (_, { payload }) => ({
        ...initialState,
        ...payload,
      }));
      ...
      builder.addCase(
        createNewVisits,
        ({ nonOrphanVisits, orphanVisits, ...rest }, { payload }) => {
          const {
            nonOrphanVisits: newNonOrphanVisits,
            orphanVisits: newOrphanVisits,
          } = groupNewVisitsByType(payload.createdVisits);

          const newNonOrphanTotalVisits = newNonOrphanVisits.length;
          const newNonOrphanBotVisits = countBots(newNonOrphanVisits);
          const newNonOrphanNonBotVisits =
            newNonOrphanTotalVisits - newNonOrphanBotVisits;

          const newOrphanTotalVisits = newOrphanVisits.length;
          const newOrphanBotVisits = countBots(newOrphanVisits);
          const newOrphanNonBotVisits =
            newOrphanTotalVisits - newOrphanBotVisits;
          ...
          return {
            ...
            nonOrphanVisits: {
              ...
              bots:
                nonOrphanVisits.bots &&
                nonOrphanVisits.bots + newNonOrphanBotVisits,
              nonBots:
                nonOrphanVisits.nonBots &&
                nonOrphanVisits.nonBots + newNonOrphanNonBotVisits,
            },
            orphanVisits: {
              total: orphanVisits.total + newOrphanTotalVisits,
              bots: orphanVisits.bots && orphanVisits.bots + newOrphanBotVisits,
              nonBots:
                orphanVisits.nonBots &&
                orphanVisits.nonBots + newOrphanNonBotVisits,
            },
          };
        }
      );
