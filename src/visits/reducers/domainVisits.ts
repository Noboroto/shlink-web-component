...
export const getDomainVisits = (apiClientFactory: () => ShlinkApiClient) =>
  createVisitsAsyncThunk({
    ...
    createLoaders: ({ domain, options }: LoadDomainVisits) => {
      ...
      const visitsLoader = (query: ShlinkVisitsParams) =>
        apiClient.getDomainVisits(domain, query);
      const lastVisitLoader = lastVisitLoaderForLoader(
        doIntervalFallback,
        (q) => apiClient.getDomainVisits(domain, q)
      );
...
export const domainVisitsReducerCreator = (
  ...
  createVisitsReducer({
    ...
    asyncThunkCreator,
    filterCreatedVisits: ({ domain, params }, createdVisits) =>
