...
import type {
  DateRange,
  MandatoryStartDateRange,
} from '../../utils/dates/helpers/dateIntervals';
import {
  calcPrevDateRange,
...
export const isNormalizedOrphanVisit = (
  visit: NormalizedVisit
): visit is NormalizedOrphanVisit =>
...
export const groupNewVisitsByType = (
...
export const filterCreatedVisitsByShortUrl = (
  ...
  createdVisits.filter(
    ({ shortUrl, visit }) =>
      shortUrl &&
      shortUrlMatches(shortUrl, shortCode, domain) &&
      isBetween(visit.date, startDate, endDate)
  );
...
export const filterCreatedVisitsByTag = (
  ...
  createdVisits.filter(
    ({ shortUrl, visit }) =>
      shortUrl?.tags.includes(tag) && isBetween(visit.date, startDate, endDate)
  );
...
export const toApiDateRange = (
  dateRange?: DateRange
): Pick<ShlinkVisitsParams, 'startDate' | 'endDate'> => {
  const startDate =
    (dateRange?.startDate && formatIsoDate(dateRange?.startDate)) ?? undefined;
  const endDate =
    (dateRange?.endDate && formatIsoDate(dateRange?.endDate)) ?? undefined;
...
export const toApiParams = ({
  filter,
  dateRange,
}: VisitsParams): Omit<ShlinkVisitsParams, 'page' | 'itemsPerPage'> => {
...
export const isMandatoryStartDateRangeParams = (
  params: VisitsParams
): params is MandatoryStartDateRangeParams =>
  isMandatoryStartDateRange(params.dateRange);

export const paramsForPrevDateRange = ({
  dateRange,
  ...rest
}: MandatoryStartDateRangeParams): VisitsParams => ({
  ...rest,
