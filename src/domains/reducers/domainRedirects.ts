...
export const editDomainRedirects = (apiClientFactory: () => ShlinkApiClient) =>
  createAsyncThunk(
    EDIT_DOMAIN_REDIRECTS,
    async ({
      domain,
      redirects: providedRedirects,
    }: EditDomainRedirects): Promise<EditDomainRedirects> => {
      const apiClient = apiClientFactory();
      const redirects = await apiClient.editDomainRedirects({
        domain,
        ...providedRedirects,
      });
