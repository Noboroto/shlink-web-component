...
describe('domainsListReducer', () => {
  ...
  const apiClientFactory = () =>
    fromPartial<ShlinkApiClient>({ listDomains, health });
  ...
  const domains: Domain[] = [
    ...filteredDomains,
    fromPartial({ domain: 'bar', status: 'validating' }),
  ];
  ...
  const {
    reducer,
    listDomains: listDomainsAction,
    checkDomainHealth,
    filterDomains,
  } = domainsListReducerCreator(apiClientFactory, editDomainRedirectsThunk);
  ...
  describe('reducer', () => {
    ...
    it('filters domains on FILTER_DOMAINS', () => {
      expect(reducer(fromPartial({ domains }), filterDomains('oO'))).toEqual({
        domains,
        filteredDomains,
      });
    });
      ...
      (domain) => {
        ...
        expect(
          reducer(
            ...
            editDomainRedirectsThunk.fulfilled(
              editDomainRedirects,
              '',
              editDomainRedirects
            )
          ...
          filteredDomains: filteredDomains.map(
            replaceRedirectsOnDomain(editDomainRedirects)
          ),
        });
      }
    );
    ...
    it.each([['foo'], ['bar'], ['does_not_exist']])(
      'replaces status on proper domain on VALIDATE_DOMAIN',
      (domain) => {
        expect(
          reducer(
            fromPartial({ domains, filteredDomains }),
          ...
          filteredDomains: filteredDomains.map(
            replaceStatusOnDomain(domain, 'valid')
          ),
        });
      }
    );
  ...
  describe('filterDomains', () => {
    it.each([['foo'], ['bar'], ['something']])(
      'creates action as expected',
      (searchTerm) => {
