...
describe('shortUrlVisitsReducer', () => {
  ...
  const { reducer, cancelGetVisits: cancelGetShortUrlVisits } =
    shortUrlVisitsReducerCreator(
      getShortUrlVisits,
  ...
  describe('reducer', () => {
    const buildState = (data: Partial<ShortUrlVisits>) =>
      fromPartial<ShortUrlVisits>(data);
    ...
    it('stops loading and returns error when rejected', () => {
      const { loading, errorData } = reducer(
        ...
        getShortUrlVisits.rejected(
          problemDetailsError,
          '',
          fromPartial({ shortCode: '' }),
          undefined,
          undefined
        )
      );
    ...
    it('returns new progress progress changes', () => {
      const { progress } = reducer(
        undefined,
        getShortUrlVisits.progressChanged(85)
      );
    ...
    it('returns fallbackInterval when falling back to another interval', () => {
      ...
      const state = reducer(
        undefined,
        getShortUrlVisits.fallbackToInterval(fallbackInterval)
      );
    ...
    it.each([
      ...
      'clears visits when visits are deleted for active short URL',
      ({ shortCode, domain, expectedVisitsLength }) => {
        const prevState = buildState({
  ...
  describe('getShortUrlVisits', () => {
    ...
    it('performs multiple API requests when response contains more pages', async () => {
      ...
      await getShortUrlVisits({ shortCode: 'abc123', params: {}, options: {} })(
        dispatchMock,
        getState,
        {}
      );
    ...
    it.each([
      ...
      async ({ dateRange, loadPrevInterval, expectsPrevVisits }) => {
        ...
        expect(getShortUrlVisitsCall).toHaveBeenCalledTimes(
